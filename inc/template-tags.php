...
if ( ! function_exists( 'campus_housing_posted_on' ) ) :
	...
	function campus_housing_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'campus-housing' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( 'by %s', 'post author', 'campus-housing' ),
	...
	function campus_housing_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'campus-housing' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
